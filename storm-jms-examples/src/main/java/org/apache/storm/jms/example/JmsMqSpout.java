...
public class JmsMqSpout extends BaseRichSpout implements MessageListener {
    private static final Logger LOG = LoggerFactory.getLogger(JmsMqSpout.class);
    ...
    private class RecoveryTask extends TimerTask {
        ...
        public void run() {
            synchronized (JmsMqSpout.this.recoveryMutex) {
                if (JmsMqSpout.this.hasFailures()) {
                    try {
                        LOG.info("Recovering from a message failure.");
                        JmsMqSpout.this.getSession().recover();
                        JmsMqSpout.this.recovered();
                    } catch (JMSException e) {
